...
import { ShieldCheck, ArrowRight, UserCircle2, Briefcase, Eye, EyeOff } from "lucide-react";
...
export default function LoginPage() {
    ...
    // Force light mode on login screen regardless of saved theme
    useEffect(() => {
    ...
    const handleOtpChange = (index: number, rawValue: string) => {
        const value = rawValue.replace(/\D/g, "").slice(-1); // only last digit
        const next = [...otp];
        next[index] = value;
        setOtp(next);
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        // Simple frontend-only admin gate for now
        if (step === 1) {
            const trimmedEmail = email.trim().toLowerCase();
            const isAdmin =
                trimmedEmail === "admin@medestation.com" &&
                password === "Admin@123";

            if (!isAdmin) {
                setError("Invalid admin credentials. Use admin@medestation.com / Admin@123.");
                return;
            }

            // Mark session in browser storage so dashboard can check it
            if (typeof window !== "undefined") {
                window.localStorage.setItem("medestation_admin", "true");
            }
            setStep(2);
            return;
        }

        // Step 2 (MFA confirmation) complete → go to dashboard
        router.push("/");
    };
            ...
            >
                ...
                <div className="relative flex items-center gap-3 z-10">
                    <Image
                        ...
                        width={40}
                        height={40}
                        className="rounded-xl shadow-lg shadow-[#2BB673]/25 bg-white"
                    />
                    <span className="font-bold text-[#1E293B] text-xl tracking-tight">MED-Estation</span>
                </div>
            ...
            <div className="flex-1 flex flex-col justify-center items-center px-8 sm:px-12 bg-white">
                <div className="w-full max-w-md space-y-7 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex lg:hidden items-center gap-2 mb-4">
                        ...
                        />
                        <span className="font-bold text-[#1E293B]">MED-Estation</span>
                    </div>
                    ...
                    <p className="text-center text-xs text-[#94A3B8]">
                        Protected by enterprise-grade encryption · <span className="text-[#2BB673]">MED-Estation v2.0</span>
                    </p>
                </div>
            </div>
        </div>
